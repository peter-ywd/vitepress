...
export const usePosts = async ({
  ...
  try {
    let paths = [];
    await Promise.all(
      srcDir.map(async (item) => {
        const path = await fg(`${item}/**/*.md`);
        paths = [...paths, ...path.filter((md) => !md.includes("index.md"))];
      })
    );
    ...
    const posts = await Promise.all(
      paths.map(async (postPath) => {
        ...
        if (!data.permalink) {
          const pathDirs = srcDir.filter(item => {
            return postPath.includes(item);
          })
          data.permalink = `/${pathDirs[0]}/${generateString(6)}`;
          flag = true;
        }
        ...
        // rewrites
        rewrites[postPath.replace(/[+()]/g, "\\$&")] = `${data.permalink}.md`
          .slice(1)
          .replace(/[+()]/g, "\\$&");
        ...
        const contents =
          data.description ||
          removeMdPro(excerpt + "") ||
          removeMdPro(content).slice(0, autoExcerpt);
    ...
    paths.map(async (postPath) => {
      ...
      const postIndex = posts.findIndex(
        (post) => post.permalink === data.permalink
      );
      ...
      const prevDiff =
        data?.prev?.text !== prevPost?.title ||
        data?.prev?.link !== prevPost?.permalink;
      const nextDiff =
        data?.next?.text !== nextPost?.title ||
        data?.next?.link !== nextPost?.permalink;
